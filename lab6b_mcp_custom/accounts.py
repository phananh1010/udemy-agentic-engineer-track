...
def _log(msg: str) -> None:
    # MCP stdio servers must not write human text to stdout.
...
class Account(BaseModel):
    ...
    def deposit(self, amount: float):
        ...
        self.balance += amount
        _log(f"Deposited ${amount}. New balance: ${self.balance}")
        self.save()
    ...
    def withdraw(self, amount: float):
        ...
        self.balance -= amount
        _log(f"Withdrew ${amount}. New balance: ${self.balance}")
        self.save()
...
if __name__ == "__main__":
    ...
    _log(f"Current Holdings: {account.get_holdings()}", file=sys.stderr)
    _log(f"Total Portfolio Value: {account.calculate_portfolio_value()}", file=sys.stderr)
    _log(f"Profit/Loss: {account.get_profit_loss()}", file=sys.stderr)
    _log(f"Transactions: {account.list_transactions()}", file=sys.stderr)